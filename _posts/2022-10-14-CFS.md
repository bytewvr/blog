...
```
`sum_exec_runtime` will be updated with `delta_exec` directly, while `delta_exec` might be scaled before being added to `vruntime`. Checking the `calc_delta_fair` routine scaling will only occur if `load.weight` doesn't match `NICE_0_LOAD`. The unlikely macro however tells the branch predictor that this is not very probable to occur. 
```
...
echo 'r:uc_out update_curr weight=+0(%di):u64 vruntime_curr=+80(+80(%di)):u64 exec_start=+64(+80(%di)):u64' >> /sys/kernel/tracing/kprobe_events
